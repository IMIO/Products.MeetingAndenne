...
params['path'] = tool.adapted().getSearchPathFromMeetingConfig(context, params)
params['portal_type'] = tool.adapted().getSearchTypesFromMeetingConfig(context, params)
params['sort_on'] = 'Date'
...
if not results:
    ...
    write('<a href="%s" class="advancedsearchlink advanced-search">%s</a>' %
            ('@@search?sort_on=Date&sort_order=reverse',
            ts.translate(label_advanced_search, context=REQUEST)))
    write('''</li>''')
